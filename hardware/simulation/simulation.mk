...
ifeq ($(SIM_SERVER),)
	...
	ssh $(SIM_USER)@$(SIM_SERVER) "if [ ! -d $(REMOTE_SUT_DIR) ]; then mkdir -p $(REMOTE_SUT_DIR); fi"
	rsync -avz --exclude .git $(SUT_DIR) $(SIM_USER)@$(SIM_SERVER):$(REMOTE_SUT_DIR)
	bash -c "trap 'make kill-remote-sim' INT; ssh $(SIM_USER)@$(SIM_SERVER) 'cd $(REMOTE_SUT_DIR)/hardware/simulation/$(SIMULATOR); make run INIT_MEM=$(INIT_MEM) USE_DDR=$(USE_DDR) RUN_EXTMEM=$(RUN_EXTMEM) VCD=$(VCD) TEST_LOG=\"$(TEST_LOG)\"'"
ifneq ($(TEST_LOG),)
...
ifneq ($(SIM_SERVER),)
	...
	ssh $(SIM_USER)@$(SIM_SERVER) 'cd $(REMOTE_SUT_DIR); make sim-clean SIMULATOR=$(SIMULATOR)'
endif
...
ifneq ($(SIM_SERVER),)
	ssh $(SIM_USER)@$(SIM_SERVER) 'if [ ! -d $(REMOTE_SUT_DIR) ]; then mkdir -p $(REMOTE_SUT_DIR); fi'
	rsync -avz --delete --exclude .git $(SUT_DIR) $(SIM_USER)@$(SIM_SERVER):$(REMOTE_SUT_DIR)
	ssh $(SIM_USER)@$(SIM_SERVER) 'cd $(REMOTE_SUT_DIR)/hardware/simulation/$(SIMULATOR); rm -f test.log'
endif
