...
#      in the top_mem_if_ddr3b_p0_timing.tcl script.
#    * The helper routines are defined in top_mem_if_ddr3b_p0_pin_map.tcl
#
# NOTE
# ----
...
set half_period [ top_mem_if_ddr3b_p0_round_3dp [ expr $t(CK) / 2.0 ] ]

# Half of reference clock
set ref_half_period [ top_mem_if_ddr3b_p0_round_3dp [ expr $t(refCK) / 2.0 ] ]
...
set data_output_min_delay [ top_mem_if_ddr3b_p0_round_3dp [ expr - $t(wru_output_min_delay_external) - $t(wru_output_min_delay_internal)]]
...
set data_output_max_delay [ top_mem_if_ddr3b_p0_round_3dp [ expr $t(wru_output_max_delay_external) + $t(wru_output_max_delay_internal)]]
...
set data_input_max_delay [ top_mem_if_ddr3b_p0_round_3dp [ expr $t(rdu_input_max_delay_external) + $t(rdu_input_max_delay_internal) ]]
...
set data_input_min_delay [ top_mem_if_ddr3b_p0_round_3dp [ expr - $t(rdu_input_min_delay_external) - $t(rdu_input_min_delay_internal) ]]

# Minimum delay on address and command paths
set ac_min_delay [ top_mem_if_ddr3b_p0_round_3dp [ expr - $t(IH) -$fpga(tPLL_JITTER) - $fpga(tPLL_PSERR) - $board(intra_addr_ctrl_skew) + $board(addresscmd_CK_skew) - $ISI(addresscmd_hold) ]]

# Maximum delay on address and command paths
set ac_max_delay [ top_mem_if_ddr3b_p0_round_3dp [ expr $t(IS) +$fpga(tPLL_JITTER) + $fpga(tPLL_PSERR) + $board(intra_addr_ctrl_skew) + $board(addresscmd_CK_skew) + $ISI(addresscmd_setup) ]]
...
if { ! [ info exists top_mem_if_ddr3b_p0_sdc_cache ] } {
	set top_mem_if_ddr3b_p0_sdc_cache 1
	top_mem_if_ddr3b_p0_initialize_ddr_db top_mem_if_ddr3b_p0_ddr_db
} else {
	if { $debug } {
...
# following loop
set instances [ array names top_mem_if_ddr3b_p0_ddr_db ]
foreach { inst } $instances {
	if { [ info exists pins ] } {
		# Clean-up stale content
		unset pins
	}
	array set pins $top_mem_if_ddr3b_p0_ddr_db($inst)
	...
	set fpga(tDQS_PHASE_JITTER) [ expr [ get_integer_node_delay -integer $::GLOBAL_top_mem_if_ddr3b_p0_dqs_delay_chain_length -parameters {IO MAX HIGH} -src DQS_PHASE_JITTER -in_fitter ] / 1000.0 ]
	...
	set final_data_input_max_delay [ top_mem_if_ddr3b_p0_round_3dp [ expr $data_input_max_delay + $fpga(tDQS_PSERR) ]]
	set final_data_input_min_delay [ top_mem_if_ddr3b_p0_round_3dp [ expr $data_input_min_delay - $t(CK) / 2.0 + $t(QH_time) - $fpga(tDQS_PSERR) - $tJITper]]

	if { $debug } {
		post_message -type info "SDC: Jitter Parameters"
		post_message -type info "SDC: -----------------"
		post_message -type info "SDC:    DQS Phase: $::GLOBAL_top_mem_if_ddr3b_p0_dqs_delay_chain_length"
	...
	# AFI clock
	set local_pll_afi_clk [ top_mem_if_ddr3b_p0_get_or_add_clock_vseries \
		-target $pll_afi_clock \
		-suffix "afi_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_mult(PLL_AFI_CLK) \
		-divide_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_div(PLL_AFI_CLK) \
		-phase $::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_AFI_CLK) ]
	
	# DQ write clock
	set local_pll_dq_write_clk [ top_mem_if_ddr3b_p0_get_or_add_clock_vseries \
		-target $pll_dq_write_clock \
		-suffix "dq_write_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_mult(PLL_MEM_CLK) \
		-divide_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_div(PLL_MEM_CLK) \
		-phase $::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_MEM_CLK) ]
	
	# DQS write clock
	set local_pll_write_clk [ top_mem_if_ddr3b_p0_get_or_add_clock_vseries \
		-target $pll_write_clock \
		-suffix "write_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_mult(PLL_WRITE_CLK) \
		-divide_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_div(PLL_WRITE_CLK) \
		-phase $::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_WRITE_CLK) ]

	# NIOS clock
	set local_pll_avl_clock [ top_mem_if_ddr3b_p0_get_or_add_clock_vseries \
		-target $pll_avl_clock \
		-suffix "avl_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_mult(PLL_NIOS_CLK) \
		-divide_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_div(PLL_NIOS_CLK) \
		-phase $::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_NIOS_CLK) ]
	
	# I/O scan chain clock
	set local_pll_config_clock [ top_mem_if_ddr3b_p0_get_or_add_clock_vseries \
		-target $pll_config_clock \
		-suffix "config_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_mult(PLL_CONFIG_CLK) \
		-divide_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_div(PLL_CONFIG_CLK) \
		-phase $::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_CONFIG_CLK) ]	

	# Half-rate DDIO clock
	set local_pll_c2p_write_clock [ top_mem_if_ddr3b_p0_get_or_add_clock_vseries \
		-target $pll_c2p_write_clock \
		-suffix "c2p_write_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_mult(PLL_C2P_WRITE_CLK) \
		-divide_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_div(PLL_C2P_WRITE_CLK) \
		-phase $::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_C2P_WRITE_CLK) ]	
		
	# Read FIFO read clock
	set local_pll_p2c_read_clock [ top_mem_if_ddr3b_p0_get_or_add_clock_vseries \
		-target $pll_p2c_read_clock \
		-suffix "p2c_read_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_mult(PLL_P2C_READ_CLK) \
		-divide_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_div(PLL_P2C_READ_CLK) \
		-phase $::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_P2C_READ_CLK) ]		

	# Half-rate clock
	set local_pll_hr_clock [ top_mem_if_ddr3b_p0_get_or_add_clock_vseries \
		-target $pll_hr_clock \
		-suffix "hr_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_mult(PLL_HR_CLK) \
		-divide_by $::GLOBAL_top_mem_if_ddr3b_p0_pll_div(PLL_HR_CLK) \
		-phase $::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_HR_CLK) ]	


	# Pulse-generator used by DQS tracking and shadow registers
	set local_sampling_clock "${inst}|top_mem_if_ddr3b_p0_sampling_clock"
	
	set sampling_clk_mult [expr $::GLOBAL_top_mem_if_ddr3b_p0_pll_mult(PLL_C2P_WRITE_CLK) * $::GLOBAL_top_mem_if_ddr3b_p0_pll_div(PLL_WRITE_CLK)]
	set sampling_clk_div [expr $::GLOBAL_top_mem_if_ddr3b_p0_pll_div(PLL_C2P_WRITE_CLK) * $::GLOBAL_top_mem_if_ddr3b_p0_pll_mult(PLL_WRITE_CLK)]
	set sampling_clk_mult_div_gcd [top_mem_if_ddr3b_p0_gcd $sampling_clk_mult $sampling_clk_div]
	...
	set sampling_clk_phase [expr $::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_C2P_WRITE_CLK) - $::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_WRITE_CLK)/2.0]
	...
	if {$::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_NIOS_CLK) != 0} {
	...
	if {$::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_C2P_WRITE_CLK) > 0} {
	...
	set write_clock_phase_offset [ expr round($::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_WRITE_CLK) - $::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_C2P_WRITE_CLK) * 2) % 360 ]
	...
	set dq_write_clock_phase_offset [ expr round($::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_MEM_CLK) - $::GLOBAL_top_mem_if_ddr3b_p0_pll_phase(PLL_C2P_WRITE_CLK) * 2) % 360 ]
	...
	set_min_delay -from [get_registers $capture_reg] -to $fifo_wrdata_reg [ top_mem_if_ddr3b_p0_round_3dp [expr -$t(CK) + 0.2 ]]
...
if {(($::quartus(nameofexecutable) ne "quartus_fit") && ($::quartus(nameofexecutable) ne "quartus_map"))} {
	set dqs_clocks [top_mem_if_ddr3b_p0_get_all_instances_dqs_pins top_mem_if_ddr3b_p0_ddr_db]
...
add_ddr_report_command "source [list [file join [file dirname [info script]] ${::GLOBAL_top_mem_if_ddr3b_p0_corename}_report_timing.tcl]]"
