...
#      in the QDRII_MASTER_p0_timing.tcl script.
#    * The helper routines are defined in QDRII_MASTER_p0_pin_map.tcl
#
# NOTE
# ----
...
source "$script_dir/QDRII_MASTER_p0_timing.tcl"
...
set io_standard "$::GLOBAL_QDRII_MASTER_p0_io_standard CLASS I"
...
set outputDQSpathjitter [expr [get_io_standard_node_delay -dst OUTPUT_DQDQS_JITTER -io_standard $io_standard -parameters [list IO $::GLOBAL_QDRII_MASTER_p0_io_interface_type] -in_fitter]/1000.0]

# This is the proportion of the DQ-DQS write path jitter that applies to setup
set outputDQSpathjitter_setup_prop [expr [get_io_standard_node_delay -dst OUTPUT_DQDQS_JITTER_DIVISION -io_standard $io_standard -parameters [list IO $::GLOBAL_QDRII_MASTER_p0_io_interface_type] -in_fitter]/100.0]
...
set half_period [ QDRII_MASTER_p0_round_3dp [ expr $t(CYC) / 2.0 ] ]

# Half of reference clock
set ref_half_period [ QDRII_MASTER_p0_round_3dp [ expr $t(refCK) / 2.0 ] ]
...
set data_output_min_delay [ QDRII_MASTER_p0_round_3dp [ expr - $t(wru_output_min_delay_external) - $t(wru_output_min_delay_internal)]]
...
set data_output_max_delay [ QDRII_MASTER_p0_round_3dp [ expr $t(wru_output_max_delay_external) + $t(wru_output_max_delay_internal)]]
...
set data_input_max_delay [ QDRII_MASTER_p0_round_3dp [ expr $t(rdu_input_max_delay_external) + $t(rdu_input_max_delay_internal) ]]
...
set data_input_min_delay [ QDRII_MASTER_p0_round_3dp [ expr - $t(rdu_input_min_delay_external) - $t(rdu_input_min_delay_internal) ]]

# Minimum delay on address and command paths
set ac_min_delay [ QDRII_MASTER_p0_round_3dp [ expr - $t(HA) - $fpga(tPLL_JITTER) - $fpga(tPLL_PSERR) - $board(intra_addr_ctrl_skew) + $board(addresscmd_CK_skew) - $ISI(addresscmd_hold)]]

# Maximum delay on address and command paths
set ac_max_delay [ QDRII_MASTER_p0_round_3dp [ expr $t(SA) + $fpga(tPLL_JITTER) + $fpga(tPLL_PSERR) + $board(intra_addr_ctrl_skew) + $board(addresscmd_CK_skew) + $ISI(addresscmd_setup)]]
...
if { ! [ info exists QDRII_MASTER_p0_sdc_cache ] } {
	set QDRII_MASTER_p0_sdc_cache 1
	QDRII_MASTER_p0_initialize_ddr_db QDRII_MASTER_p0_ddr_db
} else {
	if { $debug } {
...
# following loop
set instances [ array names QDRII_MASTER_p0_ddr_db ]
foreach { inst } $instances {
	if { [ info exists pins ] } {
		# Clean-up stale content
		unset pins
	}
	array set pins $QDRII_MASTER_p0_ddr_db($inst)
	...
	set fpga(tDQS_PHASE_JITTER) [ expr [ get_integer_node_delay -integer $::GLOBAL_QDRII_MASTER_p0_dqs_delay_chain_length -parameters {IO MAX HIGH} -src DQS_PHASE_JITTER -in_fitter ] / 1000.0 ]
	...
	set data_input_min_constraint [ QDRII_MASTER_p0_round_3dp [ expr - ($t(CYC) * 0.5 - $t(INTERNAL_JITTER) ) + $fpga(tDQS_PSERR) ]]


	# Maximum delay requirement for setup constraint on read path
	set data_input_max_constraint [ QDRII_MASTER_p0_round_3dp [ expr - $fpga(tDQS_PSERR) ]]

	if { $debug } {
		post_message -type info "SDC: Jitter Parameters"
		post_message -type info "SDC: -----------------"
		post_message -type info "SDC:    DQS Phase: $::GLOBAL_QDRII_MASTER_p0_dqs_delay_chain_length"
	...
	# AFI clock
	set local_pll_afi_clk [ QDRII_MASTER_p0_get_or_add_clock_vseries \
		-target $pll_afi_clock \
		-suffix "afi_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_QDRII_MASTER_p0_pll_mult(PLL_AFI_CLK) \
		-divide_by $::GLOBAL_QDRII_MASTER_p0_pll_div(PLL_AFI_CLK) \
		-phase $::GLOBAL_QDRII_MASTER_p0_pll_phase(PLL_AFI_CLK) ]
		
	# Write clock
	set local_pll_write_clk [ QDRII_MASTER_p0_get_or_add_clock_vseries \
		-target $pll_k_clock \
		-suffix "write_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_QDRII_MASTER_p0_pll_mult(PLL_WRITE_CLK) \
		-divide_by $::GLOBAL_QDRII_MASTER_p0_pll_div(PLL_WRITE_CLK) \
		-phase $::GLOBAL_QDRII_MASTER_p0_pll_phase(PLL_WRITE_CLK) ]		
		
 	# Half-rate DDIO clock
	set local_pll_c2p_write_clock [ QDRII_MASTER_p0_get_or_add_clock_vseries \
		-target $pll_c2p_write_clock \
		-suffix "c2p_write_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_QDRII_MASTER_p0_pll_mult(PLL_C2P_WRITE_CLK) \
		-divide_by $::GLOBAL_QDRII_MASTER_p0_pll_div(PLL_C2P_WRITE_CLK) \
		-phase $::GLOBAL_QDRII_MASTER_p0_pll_phase(PLL_C2P_WRITE_CLK) ]	
	
	# NIOS clock
	set local_pll_avl_clock [ QDRII_MASTER_p0_get_or_add_clock_vseries \
		-target $pll_avl_clock \
		-suffix "avl_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_QDRII_MASTER_p0_pll_mult(PLL_NIOS_CLK) \
		-divide_by $::GLOBAL_QDRII_MASTER_p0_pll_div(PLL_NIOS_CLK) \
		-phase $::GLOBAL_QDRII_MASTER_p0_pll_phase(PLL_NIOS_CLK) ]
	
	# I/O scan chain clock
	set local_pll_config_clock [ QDRII_MASTER_p0_get_or_add_clock_vseries \
		-target $pll_config_clock \
		-suffix "config_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_QDRII_MASTER_p0_pll_mult(PLL_CONFIG_CLK) \
		-divide_by $::GLOBAL_QDRII_MASTER_p0_pll_div(PLL_CONFIG_CLK) \
		-phase $::GLOBAL_QDRII_MASTER_p0_pll_phase(PLL_CONFIG_CLK) ]	
	...
	for { set i 0 } { $i < [ llength $k_leveling_pins ] } { incr i } {
		...
		set local_leveling_clock_k [ QDRII_MASTER_p0_get_or_add_clock_vseries \
			-target $pin_name \
	...
	for { set i 0 } { $i < [ llength $d_leveling_pins ] } { incr i } {	
		...
		set local_leveling_clock_d [ QDRII_MASTER_p0_get_or_add_clock_vseries \
			-target $pin_name \
	...
	for { set i 0 } { $i < [ llength $ac_leveling_pins ] } { incr i } {	
		...
		set local_leveling_clock_ac [ QDRII_MASTER_p0_get_or_add_clock_vseries \
			-target $pin_name \
	...
	set half_clock_constraint [ QDRII_MASTER_p0_round_3dp [ expr ($t(CYC) * 0.5 - $t(INTERNAL_JITTER) ) ]]
	...
	if {$::GLOBAL_QDRII_MASTER_p0_pll_phase(PLL_C2P_WRITE_CLK) > 0} {
	...
	if { ! $synthesis_flow } {
		...
		set_min_delay -from [get_registers $read_capture_ddio] -to $fifo_wrdata_reg [ QDRII_MASTER_p0_round_3dp [expr -$t(CYC) + 0.15]]
	}
...
if {(($::quartus(nameofexecutable) ne "quartus_fit") && ($::quartus(nameofexecutable) ne "quartus_map"))} {
	set dqs_clocks [QDRII_MASTER_p0_get_all_instances_dqs_pins QDRII_MASTER_p0_ddr_db]
...
add_ddr_report_command "source [list [file join [file dirname [info script]] ${::GLOBAL_QDRII_MASTER_p0_corename}_report_timing.tcl]]"
