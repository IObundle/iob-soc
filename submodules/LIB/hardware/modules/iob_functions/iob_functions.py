...
class iob_functions(iob_module):
    name = "iob_functions"
    version = "V0.10"
    ...
    def _copy_srcs(cls):
        ...
        shutil.copyfile(
            os.path.join(cls.setup_dir, "iob_functions.vh"),
            os.path.join(cls.build_dir, out_dir, "iob_functions.vh"),
        )
        ...
        if cls._setup_purpose[-1] == "hardware" and len(cls._setup_purpose) > 1:
            ...
            for purpose in [x for x in cls._setup_purpose[:-1] if x != "software"]:
                ...
                os.remove(
                    os.path.join(
                        cls.build_dir, cls.PURPOSE_DIRS[purpose], "iob_functions.vh"
                    )
                )
