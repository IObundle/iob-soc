...
class iob_module:
    ...
    doc_conf = ""  # CSR Configuration to use
                ...
                cls.confs.append(
                    {
                        "name": "VERSION",
                        "type": "M",
                        "val": "16'h" + copy_srcs.version_str_to_digits(cls.version) + " = " + cls.version,
                        "min": "NA",
                        "max": "NA",
    ...
    @classmethod
    def _list_all_doc_configs(cls):
        doc_configs = []
        for table in cls.regs:
    ...
    @classmethod
    def _reset_autoaddrs(cls, regs):
        if cls.autoaddr and regs:
            for table in regs:
                for r in table["regs"]:
                    r.pop("addr", None)
    ...
    def _generate_doc(cls, csr_gen_obj):
        ...
        if cls.is_top_module:
            ...
            if cls.regs:
                ...
                regs_copy = copy.deepcopy(cls.regs)
                ...
                for doc_conf in doc_configs:
                    cls._reset_autoaddrs(regs_copy)
                    _, doc_table = csr_gen_obj.get_reg_table(
                        regs_copy, cls.rw_overlap, cls.autoaddr, doc_conf
                    )
