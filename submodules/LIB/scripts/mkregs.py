...
class mkregs:
    ...
    def write_addr_and_width_defines(self, fswhdr, table, core_prefix):
    ...
    def write_swheader(self, table, out_dir, top):
        ...
        self.write_addr_and_width_defines(fswhdr, table, core_prefix)
    ...
    def write_linux_swheader(self, table, out_dir, top):
        os.makedirs(out_dir, exist_ok=True)
    ...
    def write_linux_sysfs_header(self, table, out_dir, top):
        os.makedirs(out_dir, exist_ok=True)
        ...
        fswhdr.write("static ssize_t sysfs_enosys_show(struct device *dev, struct device_attribute *attr, char *buf) {\n")
        fswhdr.write("\treturn -ENOSYS;\n")
        fswhdr.write("}\n\n")
        fswhdr.write("static ssize_t sysfs_enosys_store(struct device *dev, struct device_attribute *attr, const char __user *buf, size_t count) {\n")
        fswhdr.write("\treturn -ENOSYS;\n")
        fswhdr.write("}\n\n")
        ...
        for row in table:
            ...
            if "W" in row["type"]:
                # top, reg_name
                fswhdr.write(f"static ssize_t sysfs_{reg_name}_store(struct device *dev, struct device_attribute *attr, const char __user *buf, size_t count) {{\n")
        ...
        fswhdr.write(f"static int {top}_create_device_attr_files(struct device *device) {{\n")
        fswhdr.write("\tint ret = 0;\n")
        for row in table:
            fswhdr.write(f"\tret |= device_create_file(device, &dev_attr_{row['name'].lower()});\n")
        fswhdr.write("\treturn ret;\n")
        fswhdr.write("}\n\n")

        fswhdr.write(f"static void {top}_remove_device_attr_files(struct iob_data *{top}_data) {{\n")
        for row in table:
            fswhdr.write(f"\tdevice_remove_file({top}_data->device, &dev_attr_{row['name'].lower()});\n")
