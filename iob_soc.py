...
from iob_port import iob_interface as interface
...
class iob_soc(iob_module):
    ...
    cpu_i_bus = interface(
        ...
        wires=[
            iob_wire("cpu_i_req", width=REQ_W),
            iob_wire("cpu_i_resp", width=RESP_W)
        ],
    )
    cpu_d_bus = interface(
        name="cpu_d_bus",
        descr="Cpu data bus",
        wires=[
            iob_wire("cpu_d_req", width=REQ_W),
            iob_wire("cpu_d_resp", width=RESP_W)
        ],
    )
    ...
    int_mem_i_bus = interface(
        "int_mem_i_bus",
        descr="Internal memory instruction bus",
        wires=[
            iob_wire("int_mem_i_req", width=REQ_W),
            iob_wire("int_mem_i_resp", width=RESP_W),
        ],
    )
    # external memory instruction bus
    ext_mem_i_bus = interface(
        "ext_mem_i_bus",
        descr="External memory instruction bus",
        wires=[
            iob_wire("ext_mem_i_req", width=REQ_W),
            iob_wire("ext_mem_i_resp", width=RESP_W),
        ],
    )
    ...
    int_d_bus = interface(
        ...
        wires=[
            iob_wire("int_d_req", width=REQ_W),
            iob_wire("int_d_resp", width=RESP_W)
        ],
    )
    # external memory data bus
    ext_mem_d_bus = interface(
        "ext_mem_d_bus",
        descr="External memory data bus",
        wires=[
            iob_wire("ext_mem_d_req", width=REQ_W),
            iob_wire("ext_mem_d_resp", width=RESP_W),
        ],
    )
    ...
    slaves_bus = interface(
        "slaves_bus",
        descr="Slaves bus",
        wires=[
            iob_wire("slaves_req", width=N_SLAVES*REQ_W),
            iob_wire("slaves_resp", width=N_SLAVES*RESP_W)
        ],
    )
