...
def iob_soc_sim_setup(python_module, num_extmem_connections, exclude_files=[]):
    peripherals_list = python_module.peripherals
    confs = python_module.confs
    build_dir = python_module.build_dir
    name = python_module.name
    ...
    if not fnmatch.filter(exclude_files,'iob_soc_sim_wrapper.v'):
        create_sim_wrapper(build_dir, name, python_module.ios, confs, num_extmem_connections)
    ...
    for conf in python_module.confs:
        if (conf["name"] == "USE_EXTMEM") and conf["val"]:
            ...
            # Create extmem sim wrapper files
            iob_module.generate(
                {
...
def setup_iob_soc(python_module):
    ...
    iob_soc_sim_setup(python_module, num_extmem_connections)
