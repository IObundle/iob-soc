...
def post_setup_iob_soc(python_module, num_extmem_connections):
    ...
    if ethernet_macro:
        ...
        contents.insert(0, 'CONSOLE_CMD=$(IOB_CONSOLE_PYTHON_ENV) $(PYTHON_DIR)/console_ethernet.py -s /dev/usb-uart -c $(PYTHON_DIR)/console.py -m "$(RMAC_ADDR)"\n')
    ...
    if ethernet_macro:
        with open(python_module.build_dir + "/hardware/simulation/sim_build.mk", "r") as file:
            contents = file.readlines()
        contents.insert(0, "\n")
        # Set custom ethernet CONSOLE_CMD
        contents.insert(0, 'CONSOLE_CMD=rm -f soc2cnsl cnsl2soc; $(IOB_CONSOLE_PYTHON_ENV) $(PYTHON_DIR)/console_ethernet.py -L -c $(PYTHON_DIR)/console.py -m "$(RMAC_ADDR)"\n')
        contents.insert(0, "#Lines below were auto generated by iob_soc_utils.py\n")
        with open(python_module.build_dir + "/hardware/simulation/sim_build.mk", "w") as file:
            file.writelines(contents)
