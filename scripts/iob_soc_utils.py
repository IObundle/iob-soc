...
def post_setup_iob_soc(python_module, num_extmem_connections):
    ...
    if ethernet_macro:
        ...
        contents.insert(
            0,
            f'ETH2FILE_SCRIPT="../{name}/submodules/ETHERNET/scripts/eth2file.py"\n',
            'CONSOLE_CMD=$(IOB_CONSOLE_PYTHON_ENV) $(PYTHON_DIR)/console_ethernet.py -L -c $(PYTHON_DIR)/console.py -e $(ETH2FILE_SCRIPT) -m "$(RMAC_ADDR)"\n',
        )
        contents.insert(0, "#Lines below were auto generated by iob_soc_utils.py\n")
        with open(
