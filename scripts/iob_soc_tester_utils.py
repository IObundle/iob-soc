...
def update_tester_conf( python_module ):
    ...
    if 'submodules' in module_parameters.keys(): 
        for setup_type in ['hw_setup','sw_setup','sim_setup','fpga_setup']:
            if setup_type in module_parameters['submodules'].keys():
    ...
    submodules['dirs'].update(module_parameters['peripherals_dirs'])
    ...
    for peripheral in module_parameters['peripherals']:
