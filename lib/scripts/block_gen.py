...
def get_instance_port_connections(instance):
    ...
    for port_idx, port in enumerate(instance.ports):
        ...
        # Check if portmap was generated by if_gen (during module port generation)
        # FIXME: This does not work well. It would only work for standard portmaps (like s_s_portmap) generated by if_gen.py
        #        However, we would like to make any portmap. Not just standard ones.
        if port.name in if_gen.if_names:
            instance_portmap += f'        `include "{port.port_prefix + port.file_prefix}{port.name}{porttype}_portmap.vs"\n'
            continue
