...
if [ "$1" == "clean" ]; then
    for i in $MODULES; do 
        ...
        make clean CORE=$i CORE_DIR=$core_dir BUILD_DIR=${CUSTOM_BUILD_DIR}${i}_build
    done
    exit 0
fi
...
if [ "$1" == "test" ]; then
    for i in $MODULES; do
        ...
        make -f ${LIB_DIR}/Makefile clean setup CORE=$i CORE_DIR=$core_dir BUILD_DIR=${CUSTOM_BUILD_DIR}${i}_build
...
if [ "$1" == "build" ]; then
    ...
    make clean setup CORE=$2 CORE_DIR=$core_dir BUILD_DIR=${CUSTOM_BUILD_DIR}${2}_build
...
make clean setup CORE=$1 CORE_DIR=$core_dir BUILD_DIR=${CUSTOM_BUILD_DIR}${1}_build
