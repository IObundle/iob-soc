    ...
    def __post_init__(self):
        ...
        for inst_name, inst_info in instances.items():
            ...
            if len(port_map) != len(inst_info["module"].ports):
                raise ValueError(
                    f"Port map for instance {inst_name} is not the expected size"
                )
            for name, info in inst_info["module"].ports.items():
                ...
                if isinstance(info["width"], str):
                    ...
                    raise ValueError(
                        f"Port {name} width is not valid for instance {inst_name}"
                    )
                else:
                    width = info["width"]
                if isinstance(port_map[name], list):
                    ...
                    for name, list_range in portmap[name].items():
                        if getattr(self, name) is None:
                            raise ValueError(
                                f"No port or wire list named {name} exists"
                            )
                        ...
                        if not isinstance(getattr(self, name)[0], iob_wire):
                            raise ValueError(
                                f"Attribute {name} is not a list of wires/ports"
                            )
                        if isinstance(getattr(self, name)[0], iob_port):
                            if getattr(self, name)[0].direction != info["direction"]:
                                raise ValueError(
                                    f"Attribute {name} is not a list of {info['direction']} ports"
                                )
                        new_port_map[name].append(
                            getattr(self, name)[list_range[0] : list_range[1] + 1]
                        )
                    port_map[name] = new_port_map[name]
                else:
                    raise ValueError(
                        f"Port {name} is not a list for instance {inst_name}"
                    )
            lenght = None
            for name, items in port_map.items():
                if length is None:
                    ...
                    raise ValueError(
                        f"Port {name} list is not the same length for instance {inst_name}"
                    )
            for i in range(length):
                ...
                self.inst_list.append(
                    inst_info["module"].create(
                        inst_name,
                        inst_info["param_dict"],
                        inst_port_map,
                        inst_info["description"],
                    )
                )
