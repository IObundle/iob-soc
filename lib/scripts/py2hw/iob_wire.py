...
class iob_wire:
        ...
        if isinstance(value, str):
            ...
            if value < 0:
                if value < -(2 ** (self.width - 1)):
                    raise ValueError(
                        f"Value {value} is out of range for width {self.width}"
                    )
    ...
    def __eq__(self, other):
        if isinstance(other, iob_wire):
            if self.width != other.width:
                raise ValueError(
                    f"Cannot compare wires of different widths {self.width} and {other.width}"
                )
        else:
    ...
    def __and__(self, other):
        if isinstance(other, iob_wire):
            if self.width != other.width:
                raise ValueError(
                    f"Cannot AND wires of different widths {self.width} and {other.width}"
                )
        else:
    ...
    def __or__(self, other):
        if isinstance(other, iob_wire):
            if self.width != other.width:
                raise ValueError(
                    f"Cannot OR wires of different widths {self.width} and {other.width}"
                )
        else:
